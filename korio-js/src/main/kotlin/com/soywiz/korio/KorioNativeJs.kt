...
actual object KorioNative {
	...
	actual fun newUtcDate(fullYear: Int, month0: Int, day: Int, hours: Int, minutes: Int, seconds: Int): UTCDate {
		return UTCDateJS(fullYear, month0, day, hours, minutes, seconds)
	}
	actual fun newUtcDate(time: Long): UTCDate {
		return UTCDateJS(time)
	}
}

external private class Date(time: Double)

class UTCDateJS(time: Long) : UTCDate {
	private val date = Date(time.toDouble()).asDynamic()

	companion object {
		operator fun invoke(fullYear: Int, month0: Int, day: Int, hours: Int, minutes: Int, seconds: Int): UTCDate {
			return UTCDateJS(window["Date"].asDynamic().UTC(fullYear, month0, day, hours, minutes, seconds))
		}
	}

	override val time: Long get() = (date.getTime() as Double).toLong()
	override val fullYear: Int get() = date.getUTCFullYear()
	override val dayOfMonth: Int get() = date.getUTCDate()
	override val dayOfWeek: Int get() = date.getUTCDay()
	override val month0: Int get() = date.getUTCMonth()
	override val hours: Int get() = date.getUTCHours()
	override val minutes: Int get() = date.getUTCMinutes()
	override val seconds: Int get() = date.getUTCSeconds()
}
