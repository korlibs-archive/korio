...
actual object KorioNative {
	...
	actual fun newUtcDate(fullYear: Int, month0: Int, day: Int, hours: Int, minutes: Int, seconds: Int): UTCDate {
		return UTCDateJvm(fullYear, month0, day, hours, minutes, seconds)
	}

	actual fun newUtcDate(time: Long): UTCDate {
		return UTCDateJvm(time)
	}
}

@Suppress("DEPRECATION", "CanBeParameter")
// @TODO: Kotlin BUG: Constructor parameter is never used as a property
private class UTCDateJvm(override val time: Long) : UTCDate {
	companion object {
		private val UTC = TimeZone.getTimeZone("UTC")

		operator fun invoke(fullYear: Int, month0: Int, day: Int, hours: Int, minutes: Int, seconds: Int): UTCDate {
			return UTCDateJvm(GregorianCalendar(fullYear, month0, day, hours, minutes, seconds).apply {
				timeZone = UTC
			}.timeInMillis)
		}
	}
	...
	val jdate = GregorianCalendar.getInstance(UTC).apply {
		timeInMillis = this@UTCDateJvm.time
	}

	override val fullYear: Int get() = jdate.get(Calendar.YEAR)
	override val dayOfMonth: Int get() = jdate.get(Calendar.DAY_OF_MONTH)
	override val dayOfWeek: Int get() = jdate.get(Calendar.DAY_OF_WEEK) - 1
	override val month0: Int get() = jdate.get(Calendar.MONTH)
	override val hours: Int get() = jdate.get(Calendar.HOUR)
	override val minutes: Int get() = jdate.get(Calendar.MINUTE)
	override val seconds: Int get() = jdate.get(Calendar.SECOND)
}
