...
abstract class Vfs {
    ...
    private val readSpecialMap = hashMapOf<Class<*>, (file: VfsFile, onProgress: (Long, Long) -> Unit) -> Promise<Any>>()
    private val writeSpecialMap = hashMapOf<Class<*>, (file: VfsFile, data: Any, onProgress: (Long, Long) -> Unit) -> Promise<Unit>>()

    inline fun <reified T : Any> registerReadSpecial(noinline read: (file: VfsFile, onProgress: (Long, Long) -> Unit) -> Promise<T>) = registerReadSpecial(T::class.java, read)
    inline fun <reified T : Any> registerWriteSpecial(noinline write: (file: VfsFile, data: T, onProgress: (Long, Long) -> Unit) -> Promise<Unit>) = registerWriteSpecial(T::class.java, write)

    @Suppress("UNCHECKED_CAST")
    fun <T> registerReadSpecial(clazz: Class<T>, read: (file: VfsFile, onProgress: (Long, Long) -> Unit) -> Promise<T>) {
        readSpecialMap[clazz] = read as (file: VfsFile, onProgress: (Long, Long) -> Unit) -> Promise<Any>
    }

    @Suppress("UNCHECKED_CAST")
    fun <T> registerWriteSpecial(clazz: Class<T>, write: (file: VfsFile, data: T, onProgress: (Long, Long) -> Unit) -> Promise<Unit>) {
        writeSpecialMap[clazz] = write as (file: VfsFile, data: Any, onProgress: (Long, Long) -> Unit) -> Promise<Unit>
    }

    @Suppress("UNCHECKED_CAST")
    suspend open fun <T : Any> readSpecial(path: String, clazz: Class<T>, onProgress: (Long, Long) -> Unit = { _, _ -> }): T = asyncFun {
        val reader = readSpecialMap[clazz] ?: throw UnsupportedOperationException()
        reader(file(path), onProgress).await() as T
    }

    suspend open fun <T : Any> writeSpecial(path: String, clazz: Class<T>, value: T, onProgress: (Long, Long) -> Unit = { _, _ -> }): Unit = asyncFun {
        val writer = writeSpecialMap[value.javaClass] ?: throw UnsupportedOperationException()
        writer(file(path), value, onProgress).await()
    }
    ...
    abstract class Proxy : Vfs() {
        ...
        suspend override fun <T : Any> readSpecial(path: String, clazz: Class<T>, onProgress: (Long, Long) -> Unit): T = asyncFun { initOnce(); access(path).readSpecial(clazz, onProgress) }
        suspend override fun <T : Any> writeSpecial(path: String, clazz: Class<T>, value: T, onProgress: (Long, Long) -> Unit) = asyncFun { initOnce(); access(path).writeSpecial(clazz, value, onProgress) }
