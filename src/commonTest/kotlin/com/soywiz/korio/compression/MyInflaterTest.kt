...
class MyInflaterTest {
	@Test
	fun test() {
		val result = "CB 48 CD C9 C9 57 28 CF 2F CA 49 01 00".unhexIgnoreSpaces.uncompress(Deflate)
		assertEquals("hello world", result.toString(UTF8))
	}

	@Test
	fun test2() {
		val result = "CB 48 CD C9 C9 57 28 CF 2F CA 49 D1 51 C8 20 86 03 00".unhexIgnoreSpaces.uncompress(Deflate)
	...
	fun test2Stream() = suspendTest {
		...
		val result = Deflate.uncompressStream(input).readAll()
		assertEquals("hello world, hello world, hello world, hello world", result.toString(UTF8))
	}
